...
// 	Item.count({user: id, tags: {$in: [tag.name]}}).then(function(count) {
...
//     Item.count({user: id, tags: {$in: [tag.name]}}, function(err, count) {
//       if (err) {
...
//   async.each(tags, function(tag, done) {
//     Item.count({slackTeamId: teamId, tags: {$in: [tag.name]}}, function(err, count) {
//       if (err) {
//         return res.status(500).end();
//       }
//       var emberTag = tag.makeEmberTag(count);
...
// 			Tag.findOne({id: req.body.tag.name, user: req.body.tag.user}, function(err, data) {
// 				if (data) {
// 					res.status(400).end();
// 				}
// 				else {
