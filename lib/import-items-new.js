...
function saveAllTweetsAndUpdateUser(user, tweets, tweetArr, newestTweetId, oldestTweetId, done) {
  ...
  async.each(tweetArr, saveOneTweet, function(err) {
    if (err) {
      ...
      updateUserTweetIds(user, newestTweetId, oldestTweetId, done);
    }
  });
}

function updateUserTweetIds(user, items, newestTweetId, oldestTweetId, done) {
  User.findOneAndUpdate({id: user.id}).exec().then(function(user) {
    if(!user) {
			return done(new Error('User Not Found'));
		}
    user.twitterNewestTweetId = newestTweetId;
    user.twitterOldestTweetId = oldestTweetId;
