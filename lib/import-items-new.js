...
var fetchRelevantTweets = function(user, options) {
  ...
  BPromise.join(userLikesPromise, userTimelinePromise, function(favs, timeline) {
    var tweetsReceived = [];
...
module.exports.importTwitterItems = function(user, options, done) {
  ...
  fetchRelevantTweets(user, options)
    .then(function(arrayOfArrays) {
      updateUserIds();
      var tweetArr = arrayOfArrays[0].concat(arrayOfArrays[1]);
...
function updateUserIds(user, arrayOfArrays) {
  var latestLikeId = arrayOfArrays[0][0];
  var oldestLikeId = arrayOfArrays[0][arrayOfArrays[0].length-1];
  var latestTweetId = arrayOfArrays[1][0];
  var oldestTweetId = arrayOfArrays[1][arrayOfArrays[1].length-1];
