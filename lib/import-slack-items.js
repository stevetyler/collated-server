...
const fs = require('fs');
const exportName = './data-import/slack/collated-dev-slack-export-aug-24-2016';
...
let userDataArr = JSON.parse(fs.readFileSync(exportName + '/users.json', 'utf8'));
...
let messageArrArr = channelObj.names.reduce((arr, channelName, i) => {
  ...
  let teamId = JSON.parse(fs.readFileSync(exportName + '/users.json', 'utf8'))[0].team_id;
