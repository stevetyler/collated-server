...
Helpers.prototype.authSuccessRedirect = function(req, res) {
  let withAccountPath = process.env.NODE_ENV === 'production' ?
	...
	try {
      ...
      let token = this.extractPassportId(cookie);

			res.redirect('net.collated.ios://' + token); // connect.sid cookie
		}
		else {
...
Helpers.prototype.extractPassportId = function(str) {
  let arr = str.split(';');
