...
Helpers.prototype.authCookieOptions = {
  expires: new Date(Date.now() + 600000),
  httpOnly: true
};

Helpers.prototype.authSuccessRedirect = function(req, res) {
  //console.log('auth success called, cookie', req.headers.cookie);
  let withAccountPath = process.env.NODE_ENV === 'production' ?
	...
	try {
      ...
      let token = Helpers.prototype.extractSid(cookie);
			//let token = req.user.apiKeys.collatedToken;
      console.log('ios true, cookie, token', cookie, token);
    ...
    console.log('auth success err', err);
  }
};
...
Helpers.prototype.extractSid = function(str) {
  console.log('extract sid called');
  let arr = str.split(';');

  return arr.reduce((string, str)  => {
    var i = str.indexOf('connect.sid');

    if (i > -1) {
      return string + str.split(' ').join('').replace('connect.sid=', '');
    }
    else {
      return string + '';
    }
