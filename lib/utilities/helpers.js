...
Helpers.prototype.authSuccessRedirect = function(req, res) {
  ...
  console.log('auth success redirect called from helpers');
	try {
    if (req.headers.cookie.indexOf('android=true') > -1) {
			let token = req.user.apiKeys.collatedToken;
      let userId = req.user.id;

      console.log('android true for user ', userId, token);
      // cannot redirect straight to Android schema and needs to redirect to url for user to action due to Android restriction
      res.redirect('https://app.collated.net/twitter-android?=' + token);
      //res.redirect('net.collated.android://' + token);
		}
		else if (req.headers.cookie.indexOf('ios=true') > -1) {
			let token = req.user.apiKeys.collatedToken;
      console.log('ios true', token);
			res.redirect('net.collated.ios://' + token);
		}
		else {
      console.log('calling withAccount from helpers');
