...
module.exports.importItems = function(user, options, done) {
  ...
  client.get('favorites/list', params, function(error, tweets, response) {
    ...
    if (newestTweetId && options.getPreviousLikes === 'true') {
    ...
    if (options.getLatestLikes === 'true') {
      ...
      tweets.pop(); // remove duplicate since_id at the end
      console.log('tweets length', tweets.length, 'new tweets length', tweets.length);
    }
    if (options.getPreviousLikes === 'true') {
