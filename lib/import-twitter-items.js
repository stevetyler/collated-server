...
function saveTweetItem(twitterItem) {
  ...
  const options = {
    userId: twitterItem.user
  };

  return Item.findCategoryAndTags(textToSearch, options)
  .then(tagsObj => {
    console.log('tags found for tweet', tagsObj);
    return Object.assign({}, twitterItem, {category: tagsObj.categoryId, tags: tagsObj.tagIds});
  }).then((item) => {
