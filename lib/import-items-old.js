...
module.exports.importItems = function(user, options, done) {
  ...
  var newestTweetId = user.twitterProfile.newestTweetId;
  var oldestTweetId = user.twitterProfile.oldestTweetId;
  var params = {
    screen_name: user.id,
  };
  var tweetArr = [];
  ...
  client.get('favorites/list', params, function(error, tweets, response) {
    ...
    if (options.getLatestLikes === 'true' && user.twitterProfile.newestTweetId) {
    ...
    async.each(tweetArr, saveOneTweet, function(err) {
      if (err) {
        ...
        User.findOneAndUpdate({id: user.id}, {
          twitterProfile.newestTweetId: newestTweetId,
          twitterProfile.oldestTweetId: oldestTweetId
        }, function(err) {
          if (err) {
            return done(err);
          }
